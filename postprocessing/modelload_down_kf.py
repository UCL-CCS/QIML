...
parser = argparse.ArgumentParser(description='Load and analyze QG models')
parser.add_argument('--n_qubits', type=int, default=15, help='Number of qubits used in the model (default: 16)')
args = parser.parse_args()

data_path = "../data/kf_2d_re1000_256_120seed.npy"
gt_data = np.load(data_path)[0,:,:,:]
print(gt_data.shape)
# Calculate the global min and max values for the first 10 images
all_speeds = []
num_images = 320  # Using only the first 10 images as requested
for i in range(num_images): 
    img = gt_data[i, :, :]  # Get x component
...
# Define the same Q-Prior architecture as before
...
def load_and_generate(model_index):
    model = QG()
    model_path = f"../models/model_kf_{n_qubits}/model_{model_index}.pt"
...
def load_all_models(num_images=320):
    global_pdf_accum = np.zeros(2**n_qubits)  # 2^16 = 65536 states
    for i in range(num_images):
        try:
...
num_images = 320    # Same as in load_all_models
...
plt.show()
np.save(f"regenerated_global_kf_pdf_{n_qubits}.npy", global_pdf)
print(f"Regenerated global speed distribution has been saved to regenerated_global_kf_pdf_{n_qubits}.npy")
...
# Calculate the average Q-Prior distribution
if all_resampled_distributions:
    global_resampled_pdf = np.mean(all_resampled_distributions, axis=0)
    
    # You may need to rescale the x-axis of the Q-Prior distribution to match the ground truth
    # This assumes you want to map the Q-Prior distribution to the same range as the ground truth
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f"distribution_comparison_kf_{n_qubits}.png")
plt.show()
